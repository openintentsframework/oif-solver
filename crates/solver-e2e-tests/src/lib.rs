...
/// Creation bytecode for a 4-function Hyperlane mailbox stub used on the
/// destination chain. Equivalent to
/// `oif-contracts/test/oracle/hyperlane/HyperlaneOracle.t.sol::MailboxMock`,
/// with one essential addition: it emits `DispatchId(bytes32)` on every
/// `dispatch(...)` call. `HyperlaneSettlement::handle_transaction_confirmed`
/// (`crates/solver-settlement/src/implementations/hyperlane.rs:1187`)
/// requires a `Dispatch` or `DispatchId` event on the post-fill receipt to
/// extract the message id; the upstream MailboxMock doesn't emit either,
/// which stalls the post-fill confirmation step.
///
/// Functions exposed:
///   `dispatch(uint32, bytes32, bytes, bytes, address) payable returns (bytes32)`
///     increments `dispatchCounter`, emits `DispatchId(messageId)`,
///     returns `keccak256(abi.encode(dispatchCounter))`
///   `quoteDispatch(uint32, bytes32, bytes, bytes, address) pure returns (uint256)`
///     returns 0 (free dispatch — solver pays no gas payment)
///   `localDomain() pure returns (uint32)` — returns 1
///   `dispatchCounter() view returns (uint256)` — autogenerated getter
///
/// This bytecode is committed as an opaque artifact (the corresponding
/// Solidity source intentionally lives outside the repository). To verify
/// it: `cast call <addr> 'dispatchCounter()(uint256)'` on a freshly-deployed
/// instance returns 0.
const MOCK_MAILBOX_V2_BYTECODE: &str = include_str!("../fixtures/mock_mailbox_v2.bin");
...
sol! {
	...
	/// `dispatchCounter` is a public uint on `MailboxMock` that increments on
	/// every successful `dispatch(...)` call from `HyperlaneOracle.submit()`.
	/// Reading it from the test is the cleanest way to confirm the solver
	/// actually exercised the submit code path on the output oracle.
	#[sol(rpc)]
...
pub struct ChainDeployment {
	...
	pub token_a: Address,
	pub token_b: Address,
	/// Always-yes oracle. Used as the input oracle on the origin chain so
	/// `finalise()` can succeed without needing a real cross-chain proof.
	pub input_oracle: Address,
	/// On the destination chain this is `HyperlaneOracle` (so the solver
	/// exercises its real `submit()` code path against a stubbed mailbox).
	/// On the origin chain it's `AlwaysYesOracle` (unexercised here, since
	/// our test only flows 31337 → 31338).
	pub output_oracle: Address,
	pub input_settler: Address,
	pub output_settler: Address,
	/// `MailboxMock` deployed alongside `HyperlaneOracle` on the destination
	/// chain. `None` on the origin chain. Tests can read its
	/// `dispatchCounter()` to assert `submit()` was actually called.
...
impl Harness {
	pub async fn boot() -> Result<Self> {
		...
		// Deploy contracts on each chain. The origin chain uses AlwaysYesOracle
		// for both input + output roles (output role is unexercised since our
		// flow only goes 31337 → 31338). The destination chain swaps
		// HyperlaneOracle + MailboxMock in for the output role so the
		// solver's `submit()` path is exercised against a stubbed mailbox.
		tracing::info!("Deploying contracts to origin chain {ORIGIN_CHAIN_ID}");
		let origin = deploy_chain(
			&origin_provider,
			ORIGIN_CHAIN_ID,
			&origin_rpc,
			ChainRole::Origin,
	...
	/// Read the destination chain's `MailboxMock.dispatchCounter`. Returns
	/// `Ok(0)` if no mock mailbox was deployed (origin chain), and the
	/// current counter value otherwise. A counter > 0 after a flow finishes
	/// proves the solver actually called `HyperlaneOracle.submit()`.
	pub async fn destination_mailbox_dispatch_count(&self) -> Result<U256> {
		...
		let count = contract
			...
			.call()
			.await
			.context("read MailboxMock.dispatchCounter")?;
		Ok(count)
	}
	...
	/// orderId emitted by the `Open` event AND the receipt so callers can
	/// inspect tx hash / gas. Errors if the tx reverted.
	pub async fn user_open(&self, order: StandardOrder) -> Result<(B256, TransactionReceipt)> {
		let provider = build_provider(
			&self.origin.rpc_http,
			self.user_signer.clone(),
		)
		...
		let order_id = extract_order_id_from_receipt(&receipt)
	...
	/// and orderId topic. Once found, also fetches the tx receipt and asserts
	/// the tx didn't revert (status = 1). Returns the decoded event, the log,
	/// and the receipt so callers can pretty-print tx hashes, gas usage, etc.
	pub async fn await_event<E: SolEvent>(
		...
		order_id: B256,
		timeout: Duration,
	) -> Result<(E, Log, TransactionReceipt)> {
		...
		loop {
			...
			if let Some(log) = logs.into_iter().next() {
				...
				let tx_hash = log.transaction_hash.ok_or_else(|| {
					anyhow!("event log has no transaction_hash; cannot verify status")
				})?;
				let receipt = provider
					.get_transaction_receipt(tx_hash)
					.await
					.with_context(|| format!("get receipt for {tx_hash}"))?
					.ok_or_else(|| anyhow!("receipt missing for tx {tx_hash}"))?;
				if !receipt.status() {
					return Err(anyhow!(
						"tx {tx_hash} that emitted {} on chain {chain_id} reverted (status=0)",
						E::SIGNATURE
					));
				}
				return Ok((decoded.data, log, receipt));
			}
...
/// Loads creation bytecode from a Foundry artifact JSON. Default layout is
/// `out/<source>.sol/<contract>.json`; supply an explicit `source_file` when
/// the source filename differs from the contract name (e.g. `MailboxMock` is
/// defined inside `HyperlaneOracle.t.sol`).
fn load_bytecode(contract_name: &str) -> Result<Bytes> {
	load_bytecode_from(&format!("{contract_name}.sol"), contract_name)
}

fn load_bytecode_from(source_file: &str, contract_name: &str) -> Result<Bytes> {
	let out = oif_contracts_out()?;
	let path = out.join(source_file).join(format!("{contract_name}.json"));
...
/// Role of a chain in the test flow. The destination chain gets the
/// `HyperlaneOracle` + `MailboxMock` pair so the solver's `submit()` code
/// path is exercised; the origin keeps `AlwaysYesOracle` for the input role
/// so finalise can succeed without needing a real cross-chain proof.
...
async fn deploy_chain(
	...
	let (output_oracle, mock_mailbox) = match role {
		ChainRole::Origin => (always_yes, None),
		ChainRole::Destination => {
			let mailbox = deploy_mailbox_mock(provider).await.context("deploy MailboxMock")?;
			let hyperlane = deploy_hyperlane_oracle(provider, mailbox).await
				.context("deploy HyperlaneOracle")?;
			(hyperlane, Some(mailbox))
		},
	};
...
/// Deploy the bundled mailbox stub from `fixtures/mock_mailbox_v2.bin`. We
/// deliberately don't reuse oif-contracts's `MailboxMock` here because that
/// one doesn't emit `Dispatch`/`DispatchId`, which the solver's post-fill
/// confirmation requires (see `MOCK_MAILBOX_V2_BYTECODE` for the rationale).
async fn deploy_mailbox_mock(provider: &DynProvider) -> Result<Address> {
	let hex = MOCK_MAILBOX_V2_BYTECODE.trim().trim_start_matches("0x");
	let bytes = hex::decode(hex).context("decode mock_mailbox_v2.bin")?;
	deploy_raw(provider, Bytes::from(bytes), Bytes::new()).await
}

/// Deploy `HyperlaneOracle(mailbox, customHook, ism)`. We pass non-zero
/// placeholder addresses for `customHook` and `ism`; the mock mailbox doesn't
/// call into them, and `MailboxClient`'s constructor only requires the
/// addresses to be non-zero.
async fn deploy_hyperlane_oracle(
	provider: &DynProvider,
	mailbox: Address,
) -> Result<Address> {
	let bytecode = load_bytecode_from(
		"HyperlaneOracle.sol",
		"HyperlaneOracle",
	)?;
	// Same pattern as the upstream forge test (`makeAddr("kakaroto")`,
	// `makeAddr("karpincho")`): any non-zero address is fine for hook + ism
	// because our mock mailbox never delegates to them.
...
fn build_seed_overrides(
	...
	use solver_types::seed_overrides::{
		HyperlaneSettlementOverride, NetworkOverride, OracleOverrides,
