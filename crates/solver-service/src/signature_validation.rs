...
use crate::eip712::{compact, get_domain_separator, MessageHashComputer, SignatureValidator as EipSignatureValidator};
...
impl OrderSignatureValidator for Eip7683SignatureValidator {
    async fn validate_signature(
        ...
        let network = networks_config.get(&origin_chain_id).ok_or_else(|| {
            APIError::BadRequest {
                error_type: ApiErrorType::OrderValidationFailed,
...
impl SignatureValidationService {
    ...
    pub async fn validate_signature(
        ...
        let validator = self.validators.get(standard).ok_or_else(|| {
            APIError::BadRequest {
                error_type: ApiErrorType::OrderValidationFailed,
