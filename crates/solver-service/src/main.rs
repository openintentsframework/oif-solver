...
async fn load_config(args: &Args) -> Result<Config, Box<dyn std::error::Error>> {
    ...
    if let Some(config_path) = &args.config {
        ...
        return Config::from_file(config_path.to_str().unwrap()).await.map_err(Into::into);
    }
    ...
    if let (Some(seed_name), Some(deployment_config_str)) =
        (&args.seed, &args.deployment_config)
    {
        // Parse seed preset
        ...
        tracing::info!(
            "Merging deployment config with {} seed",
            seed_name
        );
        let merged_config = merge_config(deployment_config, seed)?;

        // Create config store for seeding
        let config_store = RedisConfigStore::with_defaults(redis_url.clone(), merged_config.solver.id.clone())?;
        ...
        if should_seed {
            ...
            tracing::info!(
                "For subsequent runs, set: export SOLVER_ID={}",
                solver_id
            );
    ...
    tracing::info!(
        "Loading configuration from Redis for solver: {}",
        solver_id
    );
