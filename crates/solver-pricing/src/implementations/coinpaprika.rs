//! CoinPaprika pricing implementation for production use.
//!
//! This implementation provides real-time asset prices from the CoinPaprika API.
//! The free tier at api.coinpaprika.com requires no API key and offers generous
//! limits, making it a solid default price source. An optional paid tier is
//! available at api-pro.coinpaprika.com: providing an `api_key` in the
//! configuration switches to the pro base URL and authenticates via the
//! `Authorization` header (raw key, no "Bearer " prefix). A custom `base_url`
//! always takes precedence over the tier-derived default.
...
/// Map of token symbols to CoinPaprika coin IDs.
///
/// CoinPaprika uses its own `{symbol}-{name}` ID scheme, so the shared
/// CoinGecko-shaped `DEFAULT_TOKEN_MAPPINGS` cannot be reused here.
pub const COINPAPRIKA_TOKEN_MAPPINGS: &[(&str, &str)] = &[
	("ETH", "eth-ethereum"),
...
/// CoinPaprika pricing implementation with caching and rate limiting.
pub struct CoinPaprikaPricing {
	/// HTTP client for API requests
	client: Client,
	/// API key for authentication (optional, switches to the api-pro tier)
	#[allow(dead_code)]
	...
	/// Timeout for the complete uncached CoinPaprika fetch path.
	request_timeout_ms: u64,
	/// Reserved timestamp for the next outbound CoinPaprika API call.
	next_api_call_at_ms: Arc<Mutex<u64>>,
}

/// CoinPaprika API response for the ticker endpoint (`/v1/tickers/{coin_id}`).
///
/// Only the `quotes` map is needed; all other ticker fields are ignored.
#[derive(Debug, Deserialize)]
struct TickerResponse {
	quotes: HashMap<String, TickerQuote>,
}

/// Quote entry inside a CoinPaprika ticker response.
#[derive(Debug, Deserialize)]
struct TickerQuote {
	price: Decimal,
}
...
impl CoinPaprikaPricing {
	/// Creates a new CoinPaprikaPricing instance with configuration.
	pub fn new(config: &serde_json::Value) -> Result<Self, PricingError> {
		// Validate configuration first
		let schema = CoinPaprikaConfigSchema;
		...
		// A custom base_url always wins, even when an api_key is configured.
		let base_url = config
			.get("base_url")
			.and_then(|v| v.as_str())
			.unwrap_or(if api_key.is_some() {
				"https://api-pro.coinpaprika.com"
			} else {
		...
		// CoinPaprika enforces monthly quotas rather than tight per-second caps,
		// so a polite 100ms spacing works for both the free and pro tiers.
		...
		// Build token ID map from the CoinPaprika-specific defaults
		let mut token_id_map: HashMap<String, String> = COINPAPRIKA_TOKEN_MAPPINGS
			.iter()
		...
		if let Some(ref key) = api_key {
			// CoinPaprika pro expects the raw key in the Authorization header.
			headers.insert(
				AUTHORIZATION,
		...
		debug!(
			...
			request_timeout_ms,
			"CoinPaprika pricing initialized"
		);
		debug!(
			"CoinPaprika token mappings: {} tokens configured",
			token_id_map.len()
		);
	...
	async fn apply_rate_limit(&self) -> Result<(), PricingError> {
		...
		if delay_ms > self.max_rate_limit_sleep_ms {
			return Err(PricingError::Network(format!(
				"CoinPaprika rate limit delay {delay_ms}ms exceeds configured maximum {}ms",
	...
	/// Get CoinPaprika coin ID for a token symbol
	fn get_coinpaprika_id(&self, symbol: &str) -> Option<String> {
		self.token_id_map.get(&symbol.to_uppercase()).cloned()
	}

	/// Check if currency is USD (the only supported quote currency)
	fn is_usd(&self, currency: &str) -> bool {
		currency.to_uppercase() == "USD"
	}

	/// Fetch a ticker from the CoinPaprika API
	async fn fetch_ticker(&self, coin_id: &str) -> Result<TickerResponse, PricingError> {
		tokio::time::timeout(
			request_timeout_duration(self.request_timeout_ms),
			self.fetch_ticker_inner(coin_id),
		)
		.await
		.map_err(|_| {
			PricingError::Network(format!(
				"CoinPaprika request timed out after {}ms",
				self.request_timeout_ms
			))
		})?
	}

	async fn fetch_ticker_inner(&self, coin_id: &str) -> Result<TickerResponse, PricingError> {
		self.apply_rate_limit().await?;

		let url = format!("{}/v1/tickers/{}?quotes=USD", self.base_url, coin_id);

		debug!("Fetching ticker from CoinPaprika API: {}", url);
	...
	async fn get_price(&self, token: &str, vs_currency: &str) -> Result<String, PricingError> {
		...
		let coinpaprika_id = self
			.get_coinpaprika_id(token)
		...
		let shared_result = in_flight_fetch
			.get_or_init(|| async {
				let response = match self.fetch_ticker(&coinpaprika_id).await {
				...
				let price = match response.quotes.get("USD").map(|quote| quote.price) {
					Some(price) => price,
					None => {
						let error =
...
#[async_trait]
impl PricingInterface for CoinPaprikaPricing {
	fn config_schema(&self) -> Box<dyn ConfigSchema> {
		Box::new(CoinPaprikaConfigSchema)
	}
...
/// Configuration schema for CoinPaprika pricing implementation.
pub struct CoinPaprikaConfigSchema;

impl ConfigSchema for CoinPaprikaConfigSchema {
...
/// Registry for CoinPaprika pricing implementation.
pub struct CoinPaprikaPricingRegistry;

impl ImplementationRegistry for CoinPaprikaPricingRegistry {
	const NAME: &'static str = "coinpaprika";
	type Factory = PricingFactory;

	fn factory() -> Self::Factory {
		create_coinpaprika_pricing
	}
}

impl PricingRegistry for CoinPaprikaPricingRegistry {}

/// Factory function for creating CoinPaprikaPricing instances.
pub fn create_coinpaprika_pricing(
	config: &serde_json::Value,
) -> Result<Box<dyn PricingInterface>, PricingError> {
	Ok(Box::new(CoinPaprikaPricing::new(config)?))
}
...
mod tests {
	...
	fn eth_ticker_body(price: f64) -> serde_json::Value {
		serde_json::json!({
			"id": "eth-ethereum",
			"name": "Ethereum",
			"symbol": "ETH",
	...
	fn test_new_default_config() {
		let pricing = CoinPaprikaPricing::new(&minimal_config()).unwrap();
		assert_eq!(pricing.base_url, "https://api.coinpaprika.com");
	...
	fn test_new_with_api_key() {
		...
		let pricing = CoinPaprikaPricing::new(&config).unwrap();
		assert_eq!(pricing.base_url, "https://api-pro.coinpaprika.com");
	}

	#[test]
	fn test_new_with_api_key_and_custom_base_url() {
		let config = serde_json::json!({
			"api_key": "pro_key_123",
			"base_url": "http://localhost:9999"
		});
		let pricing = CoinPaprikaPricing::new(&config).unwrap();
		// A custom base_url must win over the api-pro default
		assert_eq!(pricing.base_url, "http://localhost:9999");
	}

	#[test]
	fn test_coinpaprika_token_mappings() {
		// Check some key mappings exist
		assert!(COINPAPRIKA_TOKEN_MAPPINGS.contains(&("ETH", "eth-ethereum")));
		assert!(COINPAPRIKA_TOKEN_MAPPINGS.contains(&("BTC", "btc-bitcoin")));
		assert!(COINPAPRIKA_TOKEN_MAPPINGS.contains(&("USDC", "usdc-usd-coin")));
	}

	#[test]
	fn test_get_coinpaprika_id() {
		let pricing = CoinPaprikaPricing::new(&minimal_config()).unwrap();
		assert_eq!(
			pricing.get_coinpaprika_id("ETH"),
			Some("eth-ethereum".to_string())
		);
		assert_eq!(
			pricing.get_coinpaprika_id("btc"),
			Some("btc-bitcoin".to_string())
		);
		assert_eq!(
			pricing.get_coinpaprika_id("WETH"),
			Some("weth-weth".to_string())
		);
		assert_eq!(pricing.get_coinpaprika_id("UNKNOWN"), None);
	}
	...
	async fn test_custom_price_retrieval() {
		let pricing = CoinPaprikaPricing::new(&config_with_custom_prices()).unwrap();
		let result = pricing.get_price("ETH", "USD").await.unwrap();
		assert_eq!(result, "3000.50");
	}

	#[tokio::test]
	async fn test_get_price_via_custom_base_url() {
		let server = MockServer::start().await;
		Mock::given(method("GET"))
			.and(path("/v1/tickers/eth-ethereum"))
			.and(query_param("quotes", "USD"))
			.respond_with(ResponseTemplate::new(200).set_body_json(eth_ticker_body(2120.06)))
			.expect(1)
			.mount(&server)
			.await;

		let pricing = CoinPaprikaPricing::new(&serde_json::json!({
			"base_url": server.uri(),
		...
		assert_eq!(price, "2120.06");

		// Second call should be served from the cache
		let cached = pricing.get_price("ETH", "USD").await.unwrap();
		assert_eq!(cached, "2120.06");
	}
	...
	async fn test_concurrent_cold_misses_share_single_fetch() {
		...
		Mock::given(method("GET"))
			...
			.respond_with(
				ResponseTemplate::new(200)
					...
					.set_body_json(eth_ticker_body(2120.06)),
			)
			.expect(1)
			.mount(&server)
			.await;
		...
		let pricing = Arc::new(
			CoinPaprikaPricing::new(&serde_json::json!({
				"base_url": server.uri(),
				"cache_duration_seconds": 60,
				"rate_limit_delay_ms": 0
			}))
	...
	async fn test_rate_limit_sleep_cap_returns_error_without_sleeping() {
		...
		Mock::given(method("GET"))
			...
			.await;
		Mock::given(method("GET"))
			.and(path("/v1/tickers/btc-bitcoin"))
		...
		pricing.fetch_ticker("eth-ethereum").await.unwrap();

		let result = tokio::time::timeout(
			Duration::from_millis(80),
			pricing.fetch_ticker("btc-bitcoin"),
		)
		.await;
	...
	async fn test_fetch_timeout_bounds_http_stalls() {
		...
		let result = tokio::time::timeout(
			...
			pricing.fetch_ticker("eth-ethereum"),
		)
		.await;
	...
	#[tokio::test]
	async fn test_pricing_service_fallback_engages_after_coinpaprika_timeout_for_same_pair_waiters()
	{
		let server = MockServer::start().await;
		Mock::given(method("GET"))
			.and(path("/v1/tickers/eth-ethereum"))
		...
		assert_eq!(
			requests.len(),
			1,
			"same-pair timeout waiters should share one CoinPaprika attempt before fallback"
		);
	}
	...
	async fn test_stale_completed_in_flight_entry_does_not_bypass_cache_ttl() {
		...
		Mock::given(method("GET"))
			.and(path("/v1/tickers/eth-ethereum"))
			.and(query_param("quotes", "USD"))
			.respond_with(ResponseTemplate::new(200).set_body_json(eth_ticker_body(2222.0)))
			.mount(&server)
			.await;
