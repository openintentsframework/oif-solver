...
impl StorageInterface for RedisStorage {
    ...
    async fn get_batch(&self, keys: &[String]) -> Result<Vec<(String, Vec<u8>)>, StorageError> {
        ...
        debug!(requested = keys.len(), found = results.len(), "batch retrieval completed");
        Ok(results)
    }
...
pub fn create_storage(config: &toml::Value) -> Result<Box<dyn StorageInterface>, StorageError> {
    ...
    let full_redis_url = if redis_url.contains('/') && redis_url.split('/').next_back().map(|s| s.parse::<u8>().is_ok()).unwrap_or(false) {
