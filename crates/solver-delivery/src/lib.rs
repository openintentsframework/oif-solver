...
#[cfg(test)]
mod extra_native_fee_service_tests {
	use super::*;
	use alloy_primitives::U256;
	...
	struct DefaultExtraFeeDelivery;

	#[async_trait]
	impl DeliveryInterface for DefaultExtraFeeDelivery {
	...
	struct FixedExtraFeeDelivery;

	#[async_trait]
	impl DeliveryInterface for FixedExtraFeeDelivery {
		fn config_schema(&self) -> Box<dyn ConfigSchema> {
			Box::new(NoopSchema)
		}
		...
		async fn estimate_extra_native_fee(
			...
			Ok(ExtraNativeFeeEstimate {
				raw_fee_wei: "100".to_string(),
				buffer_wei: "10".to_string(),
				total_fee_wei: "110".to_string(),
			})
		}
	...
	#[tokio::test]
	async fn delivery_interface_default_extra_native_fee_is_zero() {
		let delivery = DefaultExtraFeeDelivery;

		let estimate = delivery
			.estimate_extra_native_fee(1, &sample_tx(1))
			.await
			.expect("default estimate should succeed");

		assert_eq!(estimate, ExtraNativeFeeEstimate::default());
	}

	#[tokio::test]
	async fn delivery_service_forwards_extra_native_fee_estimate() {
		let service = DeliveryService::new(
			HashMap::from([(
				10,
				Arc::new(FixedExtraFeeDelivery) as Arc<dyn DeliveryInterface>,
			)]),
			1,
			30,
			30,
		...
		let estimate = service
			.estimate_extra_native_fee(10, &sample_tx(10))
			.await
			.expect("configured chain should forward estimate");

		assert_eq!(estimate.raw_fee_wei, "100");
		assert_eq!(estimate.buffer_wei, "10");
		assert_eq!(estimate.total_fee_wei, "110");
	}

	#[tokio::test]
	async fn delivery_service_extra_native_fee_requires_registered_chain() {
		let service = DeliveryService::new(HashMap::new(), 1, 30, 30);

		let err = service
			.estimate_extra_native_fee(10, &sample_tx(10))
			.await
			.expect_err("unregistered chain should fail");

		assert!(matches!(err, DeliveryError::NoImplementationAvailable));
	}

	#[tokio::test]
	async fn default_delivery_test_backend_exercises_default_methods() {
		let delivery = DefaultExtraFeeDelivery;
		let tx = sample_tx(1);

		assert!(matches!(
			delivery.get_allowance("owner", "spender", "token", 1).await,
			Err(DeliveryError::NoImplementationAvailable)
		));
		assert!(matches!(
			delivery.get_nonce("owner", 1).await,
			Err(DeliveryError::NoImplementationAvailable)
		));
		assert!(matches!(
			delivery.get_block_number(1).await,
			Err(DeliveryError::NoImplementationAvailable)
		));
		assert!(matches!(
			delivery.estimate_gas(tx.clone()).await,
			Err(DeliveryError::NoImplementationAvailable)
		));
		assert_eq!(
			delivery
				.get_revert_data(1, tx, None, 123)
				.await
				.expect("default revert data should succeed"),
			None
		);
	}
}
